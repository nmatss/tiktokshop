...
export default async function DashboardPage() {
  ...
  const [
    ...
    // Get user's entitlements with only needed columns
    supabase
      .from('entitlements')
      .select('id, status, expires_at, course:courses(title, slug)')
      .eq('user_id', user.id),
    // Get lesson progress with nested relations
    supabase
      .from('lesson_progress')
      .select('id, completed, watched_seconds, lesson:lessons(title, slug, module:modules(title))')
      .eq('user_id', user.id)
  ...
  // Use counts only if user has active entitlement
  const totalLessons = hasActiveEntitlement ? (totalLessonsResult.count || 0) : 0
  const completedLessons = hasActiveEntitlement ? (completedLessonsResult.count || 0) : 0
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              {telegramLink && (
                <a
                  ...
                  className="group bg-dark-800 rounded-2xl border border-dark-700 p-5 hover:border-sky-500/50 transition-all"
                >
                  ...
                  <p className="text-sm text-dark-400">Grupo no Telegram</p>
                </a>
              )}
            </div>
          </div>
